import { noyal_backend } from "../../declarations/noyal_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await noyal_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await noyal_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await noyal_backend.deleteTodo(userId);
